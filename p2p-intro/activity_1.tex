...
cd lab_experiences/p2p-intro
    \end{verbatim}
    \item Crear una rama de trabajo, por ejemplo \texttt{grupo-2/p2p-intro}.
    \item Implementar \texttt{select\_next\_piece} en \texttt{node.py} y commitear en esa rama.
...
La plomería de red ya está resuelta en \texttt{node.py}. Lo pedagógico es \textbf{decidir a qué peer pedir qué pieza} (secuencial, \emph{rarest-first}, aleatorio, etc.): eso cambia mensajes, tiempos y tolerancia a fallas.
...
\textbf{10--15 min:} completar \texttt{select\_next\_piece} y commit en la rama del grupo. \textbf{15--40 min:} \texttt{node.py fetch ...}; anotar tiempos y mensajes. \\
\addlinespace
40--50 min &
Inyección de falla &
...
\vspace{0.5cm}
\noindent\textbf{Entregable mínimo (si aplica):} URL del fork, rama usada, nodos e IPs, estrategia, tiempos y mensajes, reacción ante la falla, grafo de conectividad.
